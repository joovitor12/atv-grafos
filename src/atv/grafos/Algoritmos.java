...
public class Algoritmos {
	...
	public void setAdjacentes(LinkedList<Vertice> vertices, LinkedList<Arestas> arestas,
			ArrayList<ArrayList<ListaAdjacentes>> grafo) {
		for (int i = 0; i < vertices.size(); i++) {
	...
	public void printarAdjacentes(ArrayList<ArrayList<ListaAdjacentes>> grafo) {
		for (int i = 0; i < grafo.size(); i++) {
			int vertice = i + 1;
			System.out.println("V�rtice " + vertice + " possui os seguintes adjacentes...");
			for (int j = 0; j < grafo.get(i).size(); j++) {
				System.out.println("V�rtice -> " + grafo.get(i).get(j).getVertice());
	...
	public void printMenorCaminhoBL(int tamanho, ArrayList<ArrayList<ListaAdjacentes>> grafo, int vertice,
		...
		for (int i = 0; i < tamanho; i++) {
			if (menorDistanciaBL[i] == Integer.MAX_VALUE) {
				System.out.println("Dist�ncia do v�rtice " + vertices.get(i).getId() + " para o v�rtice " + vertice
						+ " : INFINITO");
			} else if (vertices.get(i).getId() == 1) {
				System.out.println(
						"Dist�ncia do v�rtice " + vertices.get(i).getId() + " para o v�rtice " + vertice + " : 0");
			} else {
				System.out.println(
						"Dist�ncia do v�rtice " + vertices.get(i).getId() + " para o v�rtice " + vertice + " : "
								+ menorDistanciaBL[i]);
	...
	public void printMenorCaminhoDijkstra(int tamanho, ArrayList<ArrayList<ListaAdjacentes>> grafo, int vertice,
			LinkedList<Vertice> vertices) {
